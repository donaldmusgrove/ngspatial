...
// [[Rcpp::export]]
double dPoissMultiH(mat Y, List X, List M, mat offset, List beta, mat Psi, mat Delta){
...
vec betaSpois(vec beta, double sigmab, double taub, vec Y, mat X, mat M, 
              vec offset, vec psi, vec delta){
...
vec psiSpois(mat Psi, int num, double sigmap, vec Y, mat X, mat M, vec offset, 
             vec beta, vec delta, mat SigmaI){
...
vec deltaSpois(vec delta, double sigmah, double tauh, vec Y, mat X, mat M, 
               vec offset, vec beta, vec psi){
...
// [[Rcpp::export]]
SEXP msglmmPoisson(int iterations, mat Y, List X, List M, mat offset, 
                   vec sigmab, vec sigmap, vec sigmah, 
                   double taub, double nu, double taus, double tauh,
                   List beta0, mat Psi0, mat Sigma0, mat Delta0,
  ...
  for(int k=1; k < iterations+1; k++){
    ...
    for(int j=0; j<J; j++){
      ...
      bChain(k,j)  = betaj; 
      Betatemp(j)  = betaj;
      Psi.col(j)   = psiSpois(Psi, j, sigmap(j), Y.col(j), X(j), M(j), offset.col(j), bChain(k,j), Delta.col(j), SigI);
