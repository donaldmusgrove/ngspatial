...
sparse.msglmm.fit.poisson <- function(Y, X, A, M, family, beta.start, V, 
                                      offset, tol, minit, maxit, tune, 
                                      hyper, verbose){
    iterations <- minit
    n          <- nrow(Y)
    q          <- ncol(M)
    J          <- ncol(Y)
    Q          <- t(M) %*% (diag(c(rowSums(A))) - A) %*% M
    p          <- ncol(X)

    
    ### Reparameterize the SGLMM ("removes" Q from the prior)
    L  <- t(solve(chol(Q)))
    Ms <- M%*%L

    ### Check for an offset
    if (is.null(offset)){
      offset <- matrix(0, nrow=n, ncol=J)
    }
    ...
    gamma0  <- matrix(0, q, J)
    for(j in 1:J){
      fit        <- glm(Y[,j] ~ X-1+Ms+offset[,j], family=poisson)
      beta0[,j]  <- fit$coef[1:p]
      gamma0[,j] <- fit$coef[-(1:p)]
    }
    Sigma0      <- cov(gamma0)

    
    ### Check if user wants heterogeneous random effects included
    hetero <- FALSE
    ...
    ### Draw from the posterior using C++ sampler
    if(hetero){
      PP <- msglmmPoissonh(iterations, Y, X, Ms, offset, 
                           tune$sigma.b, tune$sigma.s, tune$sigma.h,
                           hyper$beta,   hyper$delta,  hyper$Sigma,
                           beta0, gamma0, delta0, Sigma0,
                           verbose)   
    } else{
      PP <- msglmmPoisson()   
    }
  ...
  repeat{
    if (hetero){
      for(j in (start + 1):(start + iterations)){
        ...
        ### Update tau.s, precision on gamma
        tau.s[j] = rgamma(1, a.s + q / 2, t(gam) %*% Q %*% gam / 2 + 1 / b.s)
        
        ### Update delta, hetero random effects
        ...
        ### Update  tau.h, precision on delta
      ...
      for (j in (start + 1):(start + iterations)){
        ...
        ### Update tau.s, precision on spatial effects gamma
        tau.s[j] = rgamma(1, a.s + q / 2, t(gam) %*% Q %*% gam / 2 + 1 / b.s)
...
#' @seealso \code{\link{residuals.sparse.msglmm}}, \code{\link{summary.sparse.msglmm}}, \code{\link{vcov.sparse.msglmm}}
#' @export
#' @examples \dontrun{
#'
...
sparse.msglmm = function(formula, family = gaussian, data, offset, A, attractive = 50, repulsive = 0, tol = 0.01, minit = 10000,
	                    maxit = 1000000, tune = list(), hyper = list(), model = TRUE, x = FALSE, y = FALSE, verbose = FALSE)
{
    ...
    fit = if (family$family == "binomial")
          ...
          else sparse.msglmm.fit.poisson(Y, X, A, M, family, nonspat$coef, V, offset, tol, minit, maxit, tune, hyper, verbose)
...
#' Return the covariance matrix of the regression parameters of a \code{sparse.msglmm} model object.
#'
...
vcov.sparse.msglmm = function(object, ...)
{
...
#'
#' @param object an object of class \code{sparse.msglmm}, typically the result of a call to \code{\link{sparse.msglmm}}.
...
#' @seealso \code{\link{sparse.msglmm}}, \code{\link{residuals.glm}}
#' @method residuals sparse.sglmm
#' @export

residuals.sparse.msglmm = function(object, type = c("deviance", "pearson", "response"), ...)
{
    type = match.arg(type)
    if (type == "response")
...
#' @param \dots additional arguments.
#' @seealso \code{\link{sparse.msglmm}}
#' @method summary sparse.msglmm
#' @export

summary.sparse.msglmm = function(object, alpha = 0.05, digits = 4, ...)
{
