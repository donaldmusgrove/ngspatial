...
vec alphaS(vec alpha, double tunea, vec Z, mat X, mat M, vec phi1){
  ...
  uP            = R::runif(0.0, 1.0);
  vec alphaProp = Rcpp::rnorm(J, 0.0, tunea);
  vec alphaP    = alphaProp + alpha;

  vec eta  = X*alpha  + M*phi1;
  vec p    = 1 + exp(-eta);
      p    = 1/p;
  
  vec etaP = X*alphaP + M*phi1;
  vec pP   = 1 + exp(-etaP);
      pP   = 1/pP;
...
vec phi1S(vec phi1, double tunep1, vec Z, mat X, mat M, vec alpha, vec phi2,
  ...
  uP           = R::runif(0.0, 1.0);
  vec phi1Prop = Rcpp::rnorm(J, 0.0, tunep1);
  vec phi1P    = phi1Prop + phi1;
...
vec betaS(vec beta, double tuneb, vec Y, vec Z, mat X, mat M, vec logE, 
  ...
  uP           = R::runif(0.0, 1.0);
  vec betaProp = Rcpp::rnorm(J, 0.0, tuneb);
  vec betaP    = betaProp + beta;

  vec eta    = logE + X*beta  + M*phi2;
  vec lambda = exp(eta);
  
  vec etaP    = logE + X*betaP  + M*phi2;
...
vec phi2S(vec phi2, double tunep2, vec Y, vec Z, mat X, mat M, vec logE, vec beta, 
  ...
  uP       = R::runif(0.0, 1.0);
  vec phi2Prop = Rcpp::rnorm(J, 0.0, tunep2);
  vec phi2P    = phi2Prop + phi2;
...
List MCMCzip(int iterations, vec Y, vec Z, mat X, mat M, vec logE,
  ...
  for(int k=1; k < iterations; k++ ){  
    ...
    p1Chain.col(k) = phi1S(p1Chain.col(k-1), tunep1, Z, X, M, aChain.col(k), p2Chain.col(k-1), SigI);
    p2Chain.col(k) = phi2S(p2Chain.col(k-1), tunep2, Y, Z, X, M, logE, bChain.col(k), p1Chain.col(k), SigI);
